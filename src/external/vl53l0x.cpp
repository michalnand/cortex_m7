...
int CVL53L0X::laser_read()
{
  ...
  if ((distance_tmp > 20) && (distance_tmp < 8192))
  {
    int k = 106;
    int q = -3616;
    int filter_alpha = 3;
    distance_result = (filter_alpha*distance_result +   (k*distance_tmp + q)/100)/(filter_alpha + 1);
  }

  return distance_result;
}

int CVL53L0X::laser_get()
{
  return distance_result;
}
